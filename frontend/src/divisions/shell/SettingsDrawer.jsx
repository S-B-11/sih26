...
import { X, Radio, Globe, RotateCcw, Check, Sparkles } from "lucide-react";
...
export default function SettingsDrawer({ isOpen, onClose }) {
  const { t, selectedLanguage, setSelectedLanguage, messages, resetSession, setIsLanguageModalOpen } = useOrca();
    ...
    <>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text)" }}>{t("settingsTitle")}</h2>
            <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 2 }}>{t("settingsSubtitle")}</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          ...
          {/* Language Selection */}
          <Section label={t("languageSection")}>
          ...
          {/* Satellite Status */}
          <Section label={t("dataSourcesSection")}>
              ...
              {SATELLITES.map((sat) => (
                <div key={sat.name} style={{
                  ...
                  <span style={{
                    ...
                    padding: "2px 8px",
                    borderRadius: 99,
                  }}>{sat.status === "Synced" ? t("synced") : t("live")}</span>
                </div>
              ))}
            </div>
          </Section>
          ...
          {/* Session History */}
          <Section label={`${t("sessionHistorySection")} (${messages.filter(m => m.sender === "user").length})`}>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 180, overflowY: "auto" }}>
              {messages.filter(m => m.sender === "user").length === 0 ? (
                <p style={{ fontSize: 12, color: "var(--text-3)", padding: "8px 0" }}>{t("noQueriesYet")}</p>
              ) : (
                messages
